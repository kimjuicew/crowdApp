...
import { Users, Armchair, UserCheck, TrendingUp, Clock } from 'lucide-react';
...
interface LibraryZonesProps {
  locationId: string;
  locationName: string;
}

export function LibraryZones({ locationId, locationName }: LibraryZonesProps) {
  ...
  useEffect(() => {
    ...
    // Subscribe to real-time updates for all seat status
    const channel = supabase
      .channel('seat_status_all')
      .on(
        'postgres_changes',
        {
          event: '*',
        ...
        () => {
          console.log('🔔 [LibraryZones] Real-time update received');
          fetchZones();
        }
      )
      .subscribe();
  ...
  const fetchZones = async () => {
    try {
      console.log('🟢 [LibraryZones] Fetching zones for all locations');
      ...
      if (error) {
        console.error('❌ [LibraryZones] Error:', error);
        return;
      }
      ...
      if (data) {
        ...
        console.log('✅ [LibraryZones] Loaded latest zones:', uniqueZones.length);
        setZones(uniqueZones);
        setLastUpdate(new Date());
      }
    } catch (err) {
      console.error('❌ [LibraryZones] Exception:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center">
        <h2 className="text-2xl font-bold text-black mb-2">{locationName}</h2>
        <p className="text-gray-600">ติดตามที่นั่งแบบเรียลไทม์</p>
      </div>
